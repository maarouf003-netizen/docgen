...
export interface ExecutedPublicEntityDto {
  ...
  /** عنوان الشخص الاعتباري أو وكيله القانوني. */
  address?: string;
  /** معرّف قيد الجهة في السجل المرجعي (من نافذة الاختيار) — يُفكّ بزر «فك الربط». */
  registryId?: number | null;
}
...
export interface ApplicantPublicEntityDto {
  ...
  governorate?: string;
  /** معرّف قيد الجهة في السجل المرجعي (من نافذة الاختيار) — يُفكّ حصرًا بزر «فك الربط». */
  registryId?: number | null;
}
...
fileNumber?: string;
  ...
  /** هل للملف استئناف منظور واحد على الأقل — لشارة «استئناف» بجانب نتائج البحث (المحسوم والمشطوب لا يُظهران الشارة). */
  hasAppeals?: boolean;
  /** معرف أول استئناف منظور على الملف عند وجوده (للانتقال إلى تفاصيل الاستئناف). */
...
export interface DelegationDto {
  ...
  /** دائرة التنفيذ المختصة بالملف المنيب (الدائرة المنيبة) — تُعرض في بطاقة «معلومات الملف المنيب» بدل الدائرة المنابة، كما هي مخزنة بلا بادئات. */
  ...
  /**
   * هل تحجب هذه الإنابة أموالها عن أي تسطير جديد؟ (محسوب خلفيًا: السارية بمناب غير نهائي؛
   * المعلّقة بلا مناب حاجبة دائمًا) — يُستخدم لترشيح قائمة الأموال في نافذة التسطير.
   */
  blocksAssets?: boolean;
  /**
   * هل بلغ الملف المناب حالة نهائية (مشطوب/مسترد/منفذ إنابة)؟ — محسوب خلفيًا (مرآة
