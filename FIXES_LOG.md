...
### أمان / موثوقية
1. **تهجير بقية مسارات القراءة إلى `useCancellableRequest`** (~8 مكوّنات: `DocumentView`, `Rotation`, `UsersActivity`, `DelegationRequests`, `ArchivedDocumentsList`...) — يشمل جلب قائمة المحامين في `TransferDocumentModal`.
...
### أداء / بنية
4. **تفكيك `DocumentService.cs`** (2682 سطرًا) إلى خدمات/partials أصغر دون تغيير العقود.
...
10. **مركزية helper ‏`stubMobile`** في `src/test/` بدل النسخ اليدوي في ~7 ملفات اختبار.
