...
### أمان / موثوقية
1. **تهجير بقية مسارات القراءة إلى `useCancellableRequest`** — **تقدم جزئي**: هُجرت `Dashboard`, `AuditLogs`, `DocumentsList`, `DelegationRequests`, `UsersActivity`, `TransferDocumentModal` (جلب المحامين). **الباقي**: `DocumentView` (جلبان)، `Rotation`, `BranchesManagement`, `ArchivedDocumentsList`، وتوحيد مودالات الإنابة الثلاثة التي تستخدم علم `cancelled` القديم (آمنة أصلًا) على الـhook الموحد.
