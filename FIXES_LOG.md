...
## إضافة جديدة: تاريخ القاء الحجز لكل أصل (2026-08-23)

- **[منجز]** حقل «تاريخ القاء الحجز» نص حر (قاعدة التواريخ: مثال 1/8/2026 + تطبيع الأرقام) لكل أنواع الأموال المنقولة وغير المنقولة، يُفسَّر زمنيًا ويُخزَّن `Asset.SeizureDate`، ويُعاد في `AssetDto.SeizureDate` بصيغة yyyy-MM-dd، ومتاح لقوالب الوورد عبر كائنات الأصول (seizure_date).
- هجرتان مطلوبتان عند النشر: `AddAssetSeizureDate` (SQLite) و`AddAssetSeizureDatePg` (Postgres) — راجع RUN_GUIDE.md §9.
commits: `23e122d` (خلفية+هجرتان) · `a497dd7` (واجهة).
