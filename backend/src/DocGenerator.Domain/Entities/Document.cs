...
public class Document : IDocumentExecutionState
{
    ...
    /// <summary>
    /// نسخة تسريع لفلترة الطالب (بوابة المندوبين): معرّف قيد السجل المرجعي لأول
    /// جهة طالبة تنفيذ مرتبطة، يُحدَّث آليًا عند كل حفظ من صفوف الجهات نفسها.
    /// ثابتة التزامن: القيود لا تُحذف فيزيائيًا من النظام (تعطيل فقط)، فلا يصير
    /// العمود قديمًا عبر مسارات التطبيق؛ أي حذف إداري مباشر يستوجب إعادة بناءه.
    /// </summary>
