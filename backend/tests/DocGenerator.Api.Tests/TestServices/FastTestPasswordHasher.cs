...
namespace DocGenerator.Api.Tests.TestServices;

/// <summary>
/// تنفيذ سريع لـ <see cref="IPasswordHasher"/> للاختبارات التكاملية فقط: نفس صيغة التخزين
/// (saltHex:hashHex عبر PBKDF2-SHA256) وبنفس سلوك التحقق، لكن بعدد تكرارات منخفض (1,000
/// بدل 200,000 في الإنتاج). السبب: كل تسجيل دخول أو إنشاء مستخدم في الاختبارات يدفع تكلفة
/// الاشتقاق كاملة، ومع العشرات من عمليات الدخول عبر كل فئة اختبارات ترتفع مدة التشغيل
/// لدقائق دون أي قيمة اختبارية إضافية — سلوك المصادقة (القفل، الحد، الإلغاء، CSRF) لا يعتمد
/// على قوة الاشتقاق. لا يُستخدم هذا التنفيذ خارج بيئة الاختبار أبدًا.
/// </summary>
